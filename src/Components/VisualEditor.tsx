...
> = ({ element, setIsActive }) => {
  ...
  Object.assign(element.style, {
    ...
    const pill: React.CSSProperties = { display:"flex", alignItems:"center", gap:6, padding:"8px 18px", borderRadius:999, border:"0.5px solid #3c3c3c", background:"rgb(36 36 36)", fontSize:13, fontWeight:500, cursor:"pointer" };
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", borderRadius:12, overflow:"hidden", fontFamily:"sans-serif", maxWidth:'300px' }}>
        {/* Canvas */}
        <div ref={canvasRef}
        ...
        {/* Toolbar */}
        <div style={{ display:"flex", alignItems:"center", borderTop:"0.5px solid #313131" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, padding:"12px 16px", borderRight:"0.5px solid #313131" }}>
...
Object.assign(element.style, {
  ...
  // shared inline styles
  const pill: React.CSSProperties = { display:"flex", alignItems:"center", gap:6, padding:"8px 18px", borderRadius:999, border:"0.5px solid #4d4d4d", background:"", fontSize:13, fontWeight:500, cursor:"pointer" };

  const toolLabel: React.CSSProperties = { fontSize:11, fontWeight:500, color:"#999", textTransform:"uppercase" as const, letterSpacing:"0.06em", whiteSpace:"nowrap" as const };

  const fitBtn = (active: boolean): React.CSSProperties => ({ padding:"5px 10px", borderRadius:6,
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", border:"0.5px solid #4d4d4d", borderRadius:12, overflow:"hidden", fontFamily:"sans-serif", maxWidth:680 }}>
      {/* Canvas */}
      <div ref={canvasRef}
        style={{ position:"relative", height:'210px', background:"#242424", overflow:"hidden", cursor: s.image ? (isDragging ? "grabbing" : "grab") : "default", ...getBgStyle(s) }}
        onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp}>
      ...
      {/* Toolbar */}
      <div style={{ display:"flex", alignItems:"center", borderTop:"0.5px solid #4d4d4d" }}>
...
type ColorMode = "background" | "color" | "borderColor" | "shadowColor";
...
const resetAllAppliedColors = () => {
  ...
  document.querySelectorAll("*").forEach((node) => {
    ...
    if (
      el.id === "__CHROMA_LENS_SELECTED_AREA__" ||
...
const [activeAlign, setActiveAlign] = useState<string>("start");
...
const btnStyle = (align: string): React.CSSProperties => ({
  padding: "8px",
  borderRadius: "6px",
  cursor: "pointer",
...
return (
    ...
    style={{minHeight:'300px', userSelect:'none'}}>
     {colors.length? '' :<div style={{display:'flex',height:'', justifyContent:'center', alignItems:'center', gap:'8px', width:'100%'}}>

     

{/* <button onClick={resetAllAppliedColors}>Reset</button> */}



<div style={{display:'flex', justifyContent:'center', alignItems:'center', gap:'8px',zIndex:2}}>
...
<Tippy  content={<span   style={{
        ...
        }}>{ShowReset?'Reset':'Edit Mode'}</span>}   
...
<div  onMouseEnter={(e) => {
    ...
    onMouseLeave={(e) => {
      ...
      <X style={{cursor:'pointer'}} color='#ffffff' size={20} /></div> :    <div></div>
}     </div>
...
id='colorSelectorBtn'
  ...
  onMouseUp={(e) => {
    ...
    e.currentTarget.style.boxShadow =
     ...
     {isActive && instructionPage? <InstructionSelectElement />:
